...
{
    FILE* file_ptr = fopen("RefRenderResult.bmp", "wb+");
...
// 3D resources
typedef float (*Brdf_func)(float3, float3, float3);
...
#define Scatter_rate 0.74
#define OBJ_TRI_COUNT 32

__device__ float scatter(float3 in_dir, float3 out_dir, float3 normal_line) {
    return Scatter_rate;
}

__device__ float mirror(float3 in_dir, float3 out_dir, float3 normal_line) {
    // norm in_dir
    float in_scale = 1 / norm3df(in_dir.x, in_dir.y, in_dir.z);
    float3 bisector = make_float3(out_dir.x - in_dir.x * in_scale, out_dir.y - in_dir.y * in_scale, out_dir.z - in_dir.z * in_scale);
    // float3 bisector = sub_float3(out_dir, in_dir);
    float norm_rate = 1 / norm3df(bisector.x, bisector.y, bisector.z);
    // return dot(bisector, normal_line) * norm_rate;
    if (dot(bisector, normal_line) * norm_rate > 0.999) {
        return 2 * PI;
    }
    else {
        return 0;
...
//     float3 bisector = sub_float3(out_dir, in_dir);
//     float norm_rate = 1 / norm3df(bisector.x, bisector.y, bisector.z);
//     if (dot(bisector, normal_line) * norm_rate > 0.95) {
//         return 1;
//     }
//     else {
//         return 0;
...
Trianle h_scene_objects[] = {
    // Light triagles
    Trianle{float3{343.0, 548.799, 227.0}, float3{343.0, 548.799, 332.0}, float3{213.0, 548.799, 332.0}, float3{0, -1, 0}, true, 0},
    Trianle{float3{343.0, 548.799, 227.0}, float3{213.0, 548.799, 227.0}, float3{213.0, 548.799, 332.0}, float3{0, -1, 0}, true, 0},
    
    // Floor
    Trianle{float3{552.8, 0.0, 0.0}, float3{0.0, 0.0, 0.0}, float3{0.0, 0.0, 559.2}, float3{0, 1, 0}, false, 0},
    Trianle{float3{552.8, 0.0, 0.0}, float3{549.6, 0.0, 559.2}, float3{0.0, 0.0, 559.2}, float3{0, 1, 0}, false, 0},

    // Ceiling
    Trianle{float3{556.0, 548.8, 0.0}, float3{556.0, 548.8, 559.2}, float3{0.0, 548.8, 559.2}, float3{0, -1, 0}, false, 0},
    Trianle{float3{556.0, 548.8, 0.0}, float3{0.0, 548.8, 0.0}, float3{0.0, 548.8, 559.2}, float3{0, -1, 0}, false, 0},

    // Back wall
    Trianle{float3{549.6, 0.0, 559.2}, float3{0.0, 0.0, 559.2}, float3{0.0, 548.8, 559.2}, float3{0, 0, -1}, false, 1},
    Trianle{float3{549.6, 0.0, 559.2}, float3{556.0, 548.8, 559.2}, float3{0.0, 548.8, 559.2}, float3{0, 0, -1}, false, 1},

    // Right wall
    Trianle{float3{0.0, 0.0, 559.2}, float3{0.0, 0.0, 0.0}, float3{0.0, 548.8, 0.0}, float3{1, 0, 0}, false, 0},
    Trianle{float3{0.0, 0.0, 559.2}, float3{0.0, 548.8, 559.2}, float3{0.0, 548.8, 0.0}, float3{1, 0, 0}, false, 0},

    // Left wall
    Trianle{float3{552.8, 0.0, 0.0}, float3{549.6, 0.0, 559.2}, float3{556.0, 548.8, 559.2}, float3{-1, 0, 0}, false, 0},
    Trianle{float3{552.8, 0.0, 0.0}, float3{556.0, 548.8, 0.0}, float3{556.0, 548.8, 559.2}, float3{-1, 0, 0}, false, 0},
    
    // Short block
    // Top
    Trianle{float3{130.0, 165.0, 65.0}, float3{82.0, 165.0, 225.0}, float3{240.0, 165.0, 272.0}, float3{0, 1, 0}, false, 0},
    Trianle{float3{130.0, 165.0, 65.0}, float3{290.0, 165.0, 114.0}, float3{240.0, 165.0, 272.0}, float3{0, 1, 0}, false, 0},
    
    // Left
    Trianle{float3{290.0, 0.0, 114.0}, float3{290.0, 165.0, 114.0}, float3{240.0, 165.0, 272.0}, float3{-0.9534, 0, -0.301709}, false, 0},
    Trianle{float3{290.0, 0.0, 114.0}, float3{240.0, 0.0, 272.0}, float3{240.0, 165.0, 272.0}, float3{-0.9534, 0, -0.301709}, false, 0},
    
    // Front
    Trianle{float3{130.0, 0.0, 65.0}, float3{130.0, 165.0, 65.0}, float3{290.0, 165.0, 114.0}, float3{-0.292826, 0, -0.956166}, false, 0},
    Trianle{float3{130.0, 0.0, 65.0}, float3{290.0, 0.0, 114.0}, float3{290.0, 165.0, 114.0}, float3{-0.292826, 0, -0.956166}, false, 0},

    // Right
    Trianle{float3{82.0, 0.0, 225.0}, float3{82.0, 165.0, 225.0}, float3{130.0, 165.0, 65.0}, float3{-0.957826, 0, -0.287348}, false, 0},
    Trianle{float3{82.0, 0.0, 225.0}, float3{130.0, 0.0, 65.0}, float3{130.0, 165.0, 65.0}, float3{-0.957826, 0, -0.287348}, false, 0},
    
    // Behind
    Trianle{float3{240.0, 0.0, 272.0}, float3{240.0, 165.0, 272.0}, float3{82.0, 165.0, 225.0}, float3{-0.285121, 0, -0.958492}, false, 0},
    Trianle{float3{240.0, 0.0, 272.0}, float3{82.0, 0.0, 225.0}, float3{82.0, 165.0, 225.0}, float3{-0.285121, 0, -0.958492}, false, 0},

    // Tall block
    // Top
    Trianle{float3{423.0, 330.0, 247.0}, float3{265.0, 330.0, 296.0}, float3{314.0, 330.0, 456.0}, float3{0, 1, 0}, false, 0},
    Trianle{float3{423.0, 330.0, 247.0}, float3{472.0, 330.0, 406.0}, float3{314.0, 330.0, 456.0}, float3{0, 1, 0}, false, 0},

    // Left
    Trianle{float3{423.0, 0.0, 247.0}, float3{423.0, 330.0, 247.0}, float3{472.0, 330.0, 406.0}, float3{0.955649, 0, -0.294508}, false, 0},
    Trianle{float3{423.0, 0.0, 247.0}, float3{472.0, 0.0, 406.0}, float3{472.0, 330.0, 406.0}, float3{0.955649, 0, -0.294508}, false, 0},

    // Behind
    Trianle{float3{472.0, 0.0, 406.0}, float3{472.0, 330.0, 406.0}, float3{314.0, 330.0, 456.0}, float3{-0.301709, 0, -0.953400}, false, 0},
    Trianle{float3{472.0, 0.0, 406.0}, float3{314.0, 0.0, 456.0}, float3{314.0, 330.0, 456.0}, float3{-0.301709, 0, -0.953400}, false, 0},

    // Right
    Trianle{float3{314.0, 0.0, 456.0}, float3{314.0, 330.0, 456.0}, float3{265.0, 330.0, 296.0}, float3{0.956166, 0, -0.292826}, false, 0},
    Trianle{float3{314.0, 0.0, 456.0}, float3{265.0, 0.0, 296.0}, float3{265.0, 330.0, 296.0}, float3{0.956166, 0, -0.292826}, false, 0},
    
    // Front
    Trianle{float3{265.0, 0.0, 296.0}, float3{265.0, 330.0, 296.0}, float3{423.0, 330.0, 247.0}, float3{-0.296209, 0, -0.955123}, false, 0},
    Trianle{float3{265.0, 0.0, 296.0}, float3{423.0, 0.0, 247.0}, float3{423.0, 330.0, 247.0}, float3{-0.296209, 0, -0.955123}, false, 0}
};
...
__device__ float shade(int object_idx, float3 src_point, float3 direction, curandState* curand_state)
{
    ...
    float scale = 1 / norm3df(direction.x, direction.y, direction.z);
    float3 out_direction = scalar_mult_float3(direction, scale); // use in BRDF
    ...
    while (true) {
        ...
        for (int i = 0; i < LIGHT_TRI_COUNT; ++i) {
            ...
            if (test_block_idx == i) {
                // printf("Hit Light!\n");
                Brdf_func brdf = NULL;
                if (d_scene_objects[src_object_idx].brdf == 0) {
                    brdf = scatter;
                }
                else {
                    brdf = mirror;
                ...
                l_dir += d_light_irradiance * brdf(scalar_mult_float3(obj_light_direction, -1), out_direction, d_scene_objects[src_object_idx].normal_line) * dot(d_scene_objects[src_object_idx].normal_line, obj_light_direction) * -1 * dot(d_scene_objects[i].normal_line, obj_light_direction) 
        ...
        if (rr_result < RR_RATE) {
            ...
            if (hit_obj_idx > -1 && !d_scene_objects[hit_obj_idx].is_light) {
                ...
                Brdf_func brdf = NULL;
                if (d_scene_objects[hit_obj_idx].brdf == 0) {
                    brdf = scatter;
                }
                else {
                    brdf = mirror;
                ...
                indir_rate = brdf(ray_direction, out_direction, d_scene_objects[hit_obj_idx].normal_line) * dot(ray_direction, d_scene_objects[hit_obj_idx].normal_line) / RR_RATE;
